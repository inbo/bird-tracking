...
* [CartoDB API documentation](http://docs.cartodb.com/cartodb-platform/sql-api.html)
...
This table contains all public occurrences recorded by the GPS trackers. The data are imported every 6 months or so from the `gps.ee_tracking_speed_limited` view in the [UvA-BiTS virtual lab](http://www.uva-bits.nl/virtual-lab/). Test records are removed and outliers flagged (see [import procedure](import-procedure.md)).
...
--- | --- | ---
cartodb_id | number | ID generated by CartoDB on record creation.
the_geom | geometry | Point (geometry) of the estimated catch position, generated by CartoDB from `longitude` and `latitude` on record creation.
the_geom_webmercator | geometry | Hidden field, generated by CartoDB from `the_geom` on record alteration, but with webmercator datum. Used to draw points on the map.
...
is_active | boolean | `False` if individual bird is confirmed or likely dead, or if we assume tracker is no longer active.
latitude | number | WGS84 latitude of the estimated catch position.
longitude | number | WGS84 longitude of the estimated catch position.
...
tracking_ended_at | date | UTC timestamp for when tracking is no longer (considered) operational.
tracking_started_at | date | UTC timestamp for when tracking is (considered) operational (= somewhat later than tagging). Used to exclude pre-tracking test records during import.
created_at | date | UTC timestamp generated by CartoDB on record creation.
updated_at | date | UTC timestamp generated by CartoDB on record alteration.

## License

All data are released to the public domain under a [CC0 waiver](https://creativecommons.org/publicdomain/zero/1.0/). This allows you to use these freely, but we would appreciate it if you [follow these norms](https://www.inbo.be/en/norms-data-use).
