...
This script prepares [CR-Birding](https://submit.cr-birding.org/) bird ringing data for upload to a [GBIF IPT](https://gbif.org/ipt).
...
data <-
  ...
  dplyr::mutate(
    ...
    bird_scientific_name = dplyr::case_match(
      bird_species,
    ...
    bird_shorthand_clean = stringr::str_remove(
      stringr::str_remove(bird_shorthand, stringr::fixed("B-")), stringr::fixed(".")
    )
  ) |>
  ...
  dplyr::filter(
    !is.na(bird_id) &
    observation_status != "onmogelijk" &
    stringr::str_sub(bird_shorthand_clean, 3, 3) != "B" # Dummy rings
  ) |>
  ...
  dplyr::mutate(
    observation_type = case_when(
      ...
      (!lead(revalidation_release) | is.na(lead(revalidation_release))) &
  ...
  dplyr::select(
    ...
    -user_id,
    -user_name,
    -`custom:sex (appearance)`,
    -`custom:reading direction`,
    -`custom:sex (copulation/courtship)`
  ) |>
...
### Create observation occurrence (resightings)
...
```{r}
readr::write_csv(occurrence, file = here::here("data", "processed", project_id, "occurrence"), na = "")
```
