...
movebank_ref_data <-
  ...
  dplyr::filter(!is.na(colour_ring)) %>%
  dplyr::mutate(
    ...
    `deploy-on-latitude` = dplyr::case_when(
      stringr::str_detect(release_location, "IJzermonding slikken") ~ 51.15146,
      TRUE ~ NA_real_
    ),
    `deploy-on-longitude` = dplyr::case_when(
      stringr::str_detect(release_location, "IJzermonding slikken") ~ 2.73127,
      TRUE ~ NA_real_
    ),
