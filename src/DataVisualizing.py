...
class TrackingVisualizer():
    ...
    def convert_dates_to_timestamps(self, indata=None, key=None):
        data = []
        for data_point in indata:
            datetime_ = data_point[key]
            timestamp = int(calendar.timegm(datetime_.utctimetuple()))
    ...
    def as_heatmap_json(self, domain='month', subdomain='day', agg_function='max'):
        ...
        if subdomain == 'day':
            ...
            reduced_data_timestamps = self.convert_dates_to_timestamps(reduced_data, key='date')
            ...
            reduced_data_timestamps = self.convert_dates_to_timestamps(reduced_data, key='day_hour')
            data_with_regular_floats = self.convert_numpydtypes_to_floats(reduced_data_timestamps, key=value_key)
