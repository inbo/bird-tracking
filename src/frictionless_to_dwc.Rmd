...
datapackage_url <- paste0("https://zenodo.org/record/", record_id, "/files/datapackage.json")
```
...
1. Get description from Zenodo API, so we have it with HTML (see #65).
...
package$licenses[[1]]$path <- license_url
```
