...
sql <- glue_sql(read_file(here("sql", "meta.sql")), .con = con)
...
```{r}
movebank_sql <- glue_sql(read_file(here("sql", "meta_movebank.sql")), .con = con)
...
```{r}
write_csv(movebank_reference_data, path = here("data", "movebank", project, "reference_data.csv"), na = "")
```
