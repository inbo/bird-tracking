...
ses_remarks_starts_with_study_site <- TRUE
year <- 2019
...
download_data(
  ...
  download_directory = here::here("data", "processed", project_id, "gps", year),
  download_filename = "movebank_gps",
  ring_numbers = ring_numbers,
  shared = shared,
  con = con,
  overwrite = overwrite_files,
  beep = FALSE # Would be annoying
)
if (beep) {
  beepr::beep(sound = "ready")
}
```
...
download_data(
  ...
  download_directory = here::here("data", "processed", project_id, "acc", year),
...
```
awk 'FNR > 1' gps_outliers/yyyy/movebank_gps_*.csv > movebank_gps_yyyy_nh.csv
```
