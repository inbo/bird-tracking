...
movebank_ref_data <-
  ...
  mutate(
    ...
    `deploy-off-timestamp` = format(
      as.POSIXct(paste(track_session_end_date, "18:00:00"), tz = "UTC", format = "%Y-%m-%d %H:%M:%S"),
      "%Y-%m-%dT%H:%M:%SZ"
    ),
    ...
    `deploy-on-timestamp` = format(
      as.POSIXct(paste(track_session_start_date, "18:00:00"), tz = "UTC", format = "%Y-%m-%d %H:%M:%S"),
      "%Y-%m-%dT%H:%M:%SZ"
    ),
    ...
    `manipulation-type` = case_when(
      release_location == "VOC_OST" ~ "other", # Hatched from egg at location and fed
      TRUE ~ "none"
    ),
