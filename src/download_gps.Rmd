...
sql <- here("sql", "individual_detections.sql")
```

Query data:

```{r}
download_detections(sql, download_directory, device_info_serials, con, FALSE)
```
...
movebank_sql <- here("sql", "movebank_gps_data.sql")
```
